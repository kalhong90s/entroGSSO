...
public class IDLE_WS_GENERATE_OTP implements IAFSubState {
	...
	private String						destNodeCommand				= EventLog.GENARATE_OTP.getEventLog();
	...
	public ArrayList<EquinoxRawData> doActionSubState(AbstractAF abstractAF, EC02Instance ec02Instance, EquinoxRawData equinoxRawData) {
		this.nextState = SubStates.IDLE_WS_GENERATE_OTP.toString();

		/************** INITIAL *****************/
		idleGenerateOTPInitInstanceAndLog(equinoxRawData, abstractAF, ec02Instance);

		/************** CODING ******************/
//		System.out.println("Start IDLE_WS_GENERATE_OTP");
		...
		if (messageValidator(rawDataIncoming)) {
			...
			this.sendWSOTPRequest = GssoDataManagement.extractGssoWSGenerateOTPRequest(rawDataIncoming);
			/** INITIAL INCOMEING **/
			incomingMessageType = IncomingMessageType.WS_GENERATE_ONETIMEPW_SOAP.getMessageType();
			...
			if (appInstance.isInquirySubSuccess()) {
				...
				if (isNoFlow) {
					rawDatasOutgoing.add(GssoDataManagement.accountTypeIsNotAllow(origInvokeProfile.getOrigEquinoxRawData()
							...
							composeSummary, SubStates.IDLE_WS_GENERATE_OTP.name()));

					/** GSSO Return GenerateOnetimePassword Response Error STATICTIC **/
...
//				if (appInstance.isWaitInquirySub() || appInstance.isWaitPortCheck()) {
				...
				/* DO InqSub IF First Messge SENT TO APP */
					...
					this.rawDatasOutgoing.add(GssoConstructMessage.createInquirySubReqToUSMPMessageFromWSGenerateOTP(rawDataIncoming, ec02Instance,
		...
		/** INVALID MESSAGE **/
			...
			rawDatasOutgoing.add(GssoConstructMessage.createReturnErrorMessageIdle(this.appInstance, appInstance.getProfile(),
					...
					logDescription, path, composeDebugLog, composeSummary, SubStates.IDLE_WS_GENERATE_OTP.name()));
	...
	private OrigInvokeProfile updateOrigInvokeProfile() {
		...
		origInvokeProfile.setCmdName(EventLog.WS_GENERATE_OTP.getEventLog());
		origInvokeProfile.setScenarioName(LogScenario.WS_GENERATE_OTP.getLogScenario());
		...
		origInvokeProfile.setGssoOrigCommand(GssoCommand.WS_GENERATE_OTP);
	...
	private boolean messageValidator(EquinoxRawData rawData) {
		...
		try {
			VerifyMessage.verifyIDLE_WS_GENERATE_OTP_Req(rawData, appInstance);
			...
			ec02Instance.incrementsStat(Statistic.GSSO_RECEIVED_BAD_GENERATEONETIMEPW_REQUEST.getStatistic());
			// ===============================================WRITE
			// DETAILS======================================================
			appInstance.getMapOrigInvokeEventDetailInput().put(rawData.getInvoke(), EventLog.WS_GENERATE_OTP.getEventLog());
			appInstance.getMapOrigInvokeDetailScenario().put(rawData.getInvoke(), LogScenario.WS_GENERATE_OTP.getLogScenario());

			try {
				/** Extract Message **/
				SendWSOTPRequest sendWSOTPRequest = GssoDataManagement.extractGssoWSGenerateOTPRequest(rawData);
			...
			if (ConfigureTool.isWriteLog(ConfigName.DEBUG_LOG_ENABLED)) {
				// ===============================================DEBUG
				// LOG==========================================================
				this.composeDebugLog.addStatisticIn(Statistic.GSSO_RECEIVED_BAD_GENERATEONETIMEPW_REQUEST.getStatistic());
			...
			// ===============================================WRITE
			// DETAILS======================================================
			appInstance.getMapOrigInvokeEventDetailOutput().put(rawData.getInvoke(), EventLog.WS_GENERATE_OTP.getEventLog());
			// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^WRITE
			// DETAILS^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
		}
	...
	private void writeLogSuccess(EquinoxRawData rawData) {
		...
		ec02Instance.incrementsStat(Statistic.GSSO_RECEIVED_GENERATEONETIMEPW_REQUEST.getStatistic());
		// ===============================================WRITE
		// DETAILS======================================================
		appInstance.getMapOrigInvokeEventDetailInput().put(rawData.getInvoke(), EventLog.WS_GENERATE_OTP.getEventLog());
		appInstance.getMapOrigInvokeDetailScenario().put(rawData.getInvoke(), LogScenario.WS_GENERATE_OTP.getLogScenario());
		...
		if (ConfigureTool.isWriteLog(ConfigName.DEBUG_LOG_ENABLED)) {
			// ===============================================DEBUG
			// LOG==========================================================
			this.composeDebugLog.addStatisticIn(Statistic.GSSO_RECEIVED_GENERATEONETIMEPW_REQUEST.getStatistic());
	...
	private void smsOrEmailFlow(EquinoxRawData rawData) {
		...
		SendWSOTPRequest sendWSOTPRequest = origInvokeProfile.getSendWSOTPRequest();

		/** FIND SERVICE TEMPLATE **/
		String oper = appInstance.getProfile().getOper();
		thisServiceTemplate = GssoDataManagement.findServiceTemplateMatchAccountType(appInstance, sendWSOTPRequest.getService(), oper);

		/** Chanel send only SMS **/

		/** CHOOSE LIFE TIMEOUT MIN **/
		GssoDataManagement.chooseDefaultValuesWSCommand(sendWSOTPRequest, thisServiceTemplate);
		...
		if (isFoundServiceTemplate) {
			...
			origInvokeProfile.setServiceKey(appInstance.getMapE01dataofService().get(sendWSOTPRequest.getService().toUpperCase())
					.getServiceKey());
	...
	private void idleGenerateOTPInitInstanceAndLog(EquinoxRawData equinoxRawData, AbstractAF abstractAF, EC02Instance ec02Instance) {
	...
	private void idleGenerateOTPSaveLog() {
		// ===============================================WRITE
		// DETAILS======================================================
