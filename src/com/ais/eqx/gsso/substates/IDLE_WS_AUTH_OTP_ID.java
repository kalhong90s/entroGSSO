...
public class IDLE_WS_AUTH_OTP_ID implements IAFSubState {
	...
	private String						destNodeCommand				= EventLog.WS_AUTHEN_OTP_ID.getEventLog();
	...
	public ArrayList<EquinoxRawData> doActionSubState(AbstractAF abstractAF, EC02Instance ec02Instance, EquinoxRawData equinoxRawData) {
		this.nextState = SubStates.IDLE_WS_AUTH_OTP_ID.toString();

		/************** INITIAL *****************/
		idleSendOTPInitInstanceAndLog(equinoxRawData, abstractAF, ec02Instance);

		/************** CODING ******************/
//		System.out.println("Start IDLE_WS_AUTH_OTP_ID");
		...
		if (messageValidator(rawDataIncoming)) {
			...
			/** EXTRACT OTP REQUEST **/
			this.sendWSOTPRequest = GssoDataManagement.extractGssoWSAuthOTPWithIDRequest(rawDataIncoming);
			
			/** INITIAL INCOMEING **/
			incomingMessageType = IncomingMessageType.WS_AUTHEN_OTP_ID_SOAP.getMessageType();
			...
			if (appInstance.isInquirySubSuccess()) {
				...
				if (isNoFlow) {
					rawDatasOutgoing.add(GssoDataManagement.accountTypeIsNotAllow(origInvokeProfile.getOrigEquinoxRawData()
							...
							composeSummary, SubStates.IDLE_WS_AUTH_OTP_ID.name()));

					/** GSSO Return WSAuthenOnetimePassword Response Error STATICTIC **/
...
//				if (appInstance.isWaitInquirySub() || appInstance.isWaitPortCheck()) {
				...
				/* DO InqSub IF First Messge SENT TO APP */
					...
					this.rawDatasOutgoing.add(GssoConstructMessage.createInquirySubReqToUSMPMessageFromWSAuthenOTPWithID(rawDataIncoming, ec02Instance,
		...
		/** INVALID MESSAGE **/
			...
			rawDatasOutgoing.add(GssoConstructMessage.createReturnErrorMessageIdle(this.appInstance, appInstance.getProfile(),
					...
					logDescription, path, composeDebugLog, composeSummary, SubStates.IDLE_WS_AUTH_OTP_ID.name()));
	...
	private OrigInvokeProfile updateOrigInvokeProfile() {
		...
		origInvokeProfile.setCmdName(EventLog.WS_AUTHEN_OTP_ID.getEventLog());
		origInvokeProfile.setScenarioName(LogScenario.WS_AUTHEN_OTP_ID.getLogScenario());
		...
		origInvokeProfile.setGssoOrigCommand(GssoCommand.WS_AUTHEN_OTP_ID);
	...
	private boolean messageValidator(EquinoxRawData rawData) {
		...
		try {
			VerifyMessage.verifyIDLE_WS_AUTHEN_OTP_ID_Req(rawData, appInstance);
			...
			ec02Instance.incrementsStat(Statistic.GSSO_RECEIVED_BAD_AUTHENONETIMEPW_W_ID_REQUEST.getStatistic());
			// ===============================================WRITE
			// DETAILS======================================================
			appInstance.getMapOrigInvokeEventDetailInput().put(rawData.getInvoke(), EventLog.WS_AUTHEN_OTP_ID.getEventLog());
			appInstance.getMapOrigInvokeDetailScenario().put(rawData.getInvoke(), LogScenario.WS_AUTHEN_OTP_ID.getLogScenario());

			try {
				/** Extract Message **/
				SendWSOTPRequest sendWSOTPRequest = GssoDataManagement.extractGssoWSAuthOTPWithIDRequest(rawData);
			...
			if (ConfigureTool.isWriteLog(ConfigName.DEBUG_LOG_ENABLED)) {
				// ===============================================DEBUG
				// LOG==========================================================
				this.composeDebugLog.addStatisticIn(Statistic.GSSO_RECEIVED_BAD_AUTHENONETIMEPW_W_ID_REQUEST.getStatistic());
			...
			// ===============================================WRITE
			// DETAILS======================================================
			appInstance.getMapOrigInvokeEventDetailOutput().put(rawData.getInvoke(), EventLog.WS_AUTHEN_OTP_ID.getEventLog());
			// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^WRITE
			// DETAILS^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
		}
	...
	private void writeLogSuccess(EquinoxRawData rawData) {
		...
		ec02Instance.incrementsStat(Statistic.GSSO_RECEIVED_AUTHENONETIMEPW_W_ID_REQUEST.getStatistic());
		// ===============================================WRITE
		// DETAILS======================================================
		appInstance.getMapOrigInvokeEventDetailInput().put(rawData.getInvoke(), EventLog.WS_AUTHEN_OTP_ID.getEventLog());
		appInstance.getMapOrigInvokeDetailScenario().put(rawData.getInvoke(), LogScenario.WS_AUTHEN_OTP_ID.getLogScenario());
		...
		if (ConfigureTool.isWriteLog(ConfigName.DEBUG_LOG_ENABLED)) {
			// ===============================================DEBUG
			// LOG==========================================================
			this.composeDebugLog.addStatisticIn(Statistic.GSSO_RECEIVED_AUTHENONETIMEPW_W_ID_REQUEST.getStatistic());
	...
	private void smsOrEmailFlow(EquinoxRawData rawData) {
		...
		/** WS_AUTHEN_OTP_ID_SOAP find otp chanel **/
		boolean isSms = false;
