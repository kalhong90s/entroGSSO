...
public class IDLE_WS_AUTH_OTP implements IAFSubState {
	...
	private String						destNodeCommand				= EventLog.WS_AUTHEN_OTP.getEventLog();
	...
	public ArrayList<EquinoxRawData> doActionSubState(AbstractAF abstractAF, EC02Instance ec02Instance, EquinoxRawData equinoxRawData) {
		this.nextState = SubStates.IDLE_WS_AUTH_OTP.toString();

		/************** INITIAL *****************/
		idleAuthenOTPInitInstanceAndLog(equinoxRawData, abstractAF, ec02Instance);

		/************** CODING ******************/
//		System.out.println("Start IDLE_WS_AUTH_OTP");
		...
		if (messageValidator(rawDataIncoming)) {
			...
			/** EXTRACT OTP REQUEST **/
			this.sendWSOTPRequest = GssoDataManagement.extractGssoWSAuthOTPRequest(rawDataIncoming);
			
			/** INITIAL INCOMEING **/
			String incomingMessageType = IncomingMessageType.WS_AUTHEN_OTP_SOAP.getMessageType();
			...
			if (appInstance.isInquirySubSuccess()) {
				...
				if (isNoFlow) {
					rawDatasOutgoing.add(GssoDataManagement.accountTypeIsNotAllow(origInvokeProfile.getOrigEquinoxRawData()
							...
							composeSummary, SubStates.IDLE_WS_AUTH_OTP.name()));
...
//				if (appInstance.isWaitInquirySub() || appInstance.isWaitPortCheck()) {
				...
				/* DO InqSub IF First Messge SENT TO APP */
					...
					this.rawDatasOutgoing.add(GssoConstructMessage.createInquirySubReqToUSMPMessageFromWSAuthenOTP(rawDataIncoming, ec02Instance,
		...
		/** INVALID MESSAGE **/
			...
			rawDatasOutgoing.add(GssoConstructMessage.createReturnErrorMessageIdle(this.appInstance, appInstance.getProfile(),
					...
					logDescription, path, composeDebugLog, composeSummary, SubStates.IDLE_WS_AUTH_OTP.name()));
	...
	private OrigInvokeProfile updateOrigInvokeProfile() {
		...
		origInvokeProfile.setCmdName(EventLog.WS_AUTHEN_OTP.getEventLog());
		origInvokeProfile.setScenarioName(LogScenario.WS_AUTHEN_OTP.getLogScenario());
		...
		origInvokeProfile.setGssoOrigCommand(GssoCommand.WS_AUTHEN_OTP);
	...
	private boolean messageValidator(EquinoxRawData rawData) {
		...
		try {
			VerifyMessage.verifyIDLE_WS_AUTHEN_OTP_Req(rawData, appInstance);
			...
			ec02Instance.incrementsStat(Statistic.GSSO_RECEIVED_BAD_AUTHENONETIMEPW_REQUEST.getStatistic());
			// ===============================================WRITE
			// DETAILS======================================================
			appInstance.getMapOrigInvokeEventDetailInput().put(rawData.getInvoke(), EventLog.WS_AUTHEN_OTP.getEventLog());
			appInstance.getMapOrigInvokeDetailScenario().put(rawData.getInvoke(), LogScenario.WS_AUTHEN_OTP.getLogScenario());

			try {
				/** Extract Message **/
				SendWSOTPRequest sendWSOTPRequest = GssoDataManagement.extractGssoWSAuthOTPRequest(rawData);
			...
			if (ConfigureTool.isWriteLog(ConfigName.DEBUG_LOG_ENABLED)) {
				// ===============================================DEBUG
				// LOG==========================================================
				this.composeDebugLog.addStatisticIn(Statistic.GSSO_RECEIVED_BAD_AUTHENONETIMEPW_REQUEST.getStatistic());
			...
			// ===============================================WRITE
			// DETAILS======================================================
			appInstance.getMapOrigInvokeEventDetailOutput().put(rawData.getInvoke(), EventLog.WS_AUTHEN_OTP.getEventLog());
			// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^WRITE
			// DETAILS^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
		}
	...
	private void writeLogSuccess(EquinoxRawData rawData) {
		...
		ec02Instance.incrementsStat(Statistic.GSSO_RECEIVED_AUTHENONETIMEPW_REQUEST.getStatistic());
		// ===============================================WRITE
		// DETAILS======================================================
		appInstance.getMapOrigInvokeEventDetailInput().put(rawData.getInvoke(), EventLog.WS_AUTHEN_OTP.getEventLog());
		appInstance.getMapOrigInvokeDetailScenario().put(rawData.getInvoke(), LogScenario.WS_AUTHEN_OTP.getLogScenario());
		...
		if (ConfigureTool.isWriteLog(ConfigName.DEBUG_LOG_ENABLED)) {
			// ===============================================DEBUG
			// LOG==========================================================
			this.composeDebugLog.addStatisticIn(Statistic.GSSO_RECEIVED_AUTHENONETIMEPW_REQUEST.getStatistic());
	...
	private void smsOrEmailFlow(EquinoxRawData rawData) {
		...
		/** WS_AUTHEN_OTP_SOAP find otp chanel **/
		boolean isSms = false;
	...
	private void idleAuthenOTPInitInstanceAndLog(EquinoxRawData equinoxRawData, AbstractAF abstractAF, EC02Instance ec02Instance) {
	...
	private void idleWSAuthenOTPSaveLog() {
		// ===============================================WRITE
		// DETAILS======================================================
