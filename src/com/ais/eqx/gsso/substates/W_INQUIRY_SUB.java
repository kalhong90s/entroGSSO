...
public class W_INQUIRY_SUB implements IAFSubState {
	...
	private InquirySubscriber		inquirySubscriber;
	...
	private String					destNodeCommand				= "InquirySubscriber";
	...
	public ArrayList<EquinoxRawData> doActionSubState(AbstractAF abstractAF, EC02Instance ec02Instance, EquinoxRawData equinoxRawData) {
		this.nextState = SubStates.W_INQUIRY_SUB.toString();
		/** W_INQUIRY_SUB **/
		...
		if (RetNumber.NORMAL.equals(equinoxRawData.getRet())) {
			...
			try {
				...
				this.inquirySubscriber = SoapToObject.parserInquirySub(rawDataInput.getRawDataMessage());

				/****** Valify Message ***********/
				VerifyUSMPMessage.inquirySubscriberValidator(this.inquirySubscriber, rawDataInput.getRawDataMessage());

				/* SET INQ SUB SUCCESS */
				appInstance.setInquirySubSuccess(true);

				String usmpCode = this.inquirySubscriber.getInquirySubscriberResult().getOperationStatus().getCode();

				if ((usmpCode.equals(USMPCode.VSMP_00000000) || usmpCode.equals(USMPCode.VSMP_01040001) || usmpCode
						.equals(USMPCode.VSMP_01040003))) {

					this.ec02Instance.incrementsStat(Statistic.GSSO_RECEIVED_USMP_INQUIRYSUBSCRIBER_RESPONSE_SUCCESS.getStatistic());

					if (ConfigureTool.isWriteLog(ConfigName.DEBUG_LOG_ENABLED)) {
						this.composeDebugLog.messageResponseSuccess(usmpCode);
						this.composeDebugLog.addStatisticIn(Statistic.GSSO_RECEIVED_USMP_INQUIRYSUBSCRIBER_RESPONSE_SUCCESS
								.getStatistic());
						this.composeDebugLog.messageResponseSuccess(usmpCode);
					}
					...
					String usmpDescription = inquirySubscriber.getInquirySubscriberResult().getOperationStatus().getDescription();

					this.destNodeResultCode = usmpCode;
					this.destNodeResultDescription = usmpDescription;

					this.ec02Instance.incrementsStat(Statistic.GSSO_RECEIVED_USMP_INQUIRYSUBSCRIBER_RESPONSE_RESULTCODE_ERROR.getStatistic());

					if (ConfigureTool.isWriteLog(ConfigName.DEBUG_LOG_ENABLED)) {
						this.composeDebugLog.addStatisticIn(Statistic.GSSO_RECEIVED_USMP_INQUIRYSUBSCRIBER_RESPONSE_RESULTCODE_ERROR.getStatistic());
					...
					ResponseMessage.returnMessage_Error(origInvokeProcess, rawDatasOutgoing, abstractAF, ec02Instance, listCode,
							listDescription, listOrigInvoke, composeDebugLog, true);
				...
				this.ec02Instance.incrementsStat(Statistic.GSSO_RECEIVED_BAD_USMP_INQUIRYSUBSCRIBER_RESPONSE.getStatistic());
				...
				if (ConfigureTool.isWriteLog(ConfigName.DEBUG_LOG_ENABLED)) {
					...
					this.composeDebugLog.addStatisticIn(Statistic.GSSO_RECEIVED_BAD_USMP_INQUIRYSUBSCRIBER_RESPONSE.getStatistic());
		...
		/** TIMEOUT **/
			...
			ec02Instance.incrementsStat(Statistic.GSSO_RECEIVED_USMP_INQUIRYSUBSCRIBER_REREQUEST_TIMEOUT.getStatistic());

			if (ConfigureTool.isWriteLog(ConfigName.DEBUG_LOG_ENABLED)) {
				composeDebugLog.addStatisticIn(Statistic.GSSO_RECEIVED_USMP_INQUIRYSUBSCRIBER_REREQUEST_TIMEOUT.getStatistic());
			...
			isSummaryEnable = ResponseMessage.returnMessage_TimeOut(origInvokeProcess, rawDatasOutgoing, abstractAF, ec02Instance,
					listCode, listDescription, listOrigInvoke, composeDebugLog, true);
		}
		/** ERROR, REJECT, ABORT **/
		else {
			this.nextState = SubStates.END.toString();
	...
	private void successCase(EquinoxRawData rawDataInput, AbstractAF abstractAF, String origInvoke) {
		...
		OperationStatusOfSub operation = this.inquirySubscriber.getInquirySubscriberResult().getOperationStatus();

		/** Code is VSMP-00000000 **/
		GssoProfile profile = this.appInstance.getProfile();
		if (operation.getCode().equals(USMPCode.VSMP_00000000)) {

			SubscriberOfSub subScriber = this.inquirySubscriber.getInquirySubscriberResult().getSubscriber();
			...
			if (Arrays.asList(active_State).contains(subScriber.getState())) {
				...
				profile.setLanguage(subScriber.getSmsLanguage());
				...
				if (GssoDataManagement.checkPermissionAccountType(origInvokeProfile, appInstance)) {
					...
					if (origInvokeProfile.getSendWSOTPRequest() != null
						...
						if ((origInvokeProfile.getSendWSOTPRequest().getOtpMobile() != null
							...
							/** Msisdn and OTPMobile is duplicate or Send second time**/
							else {
...
//		}
	...
	private void errorCase(EquinoxRawData rawData) {
		...
		if (rawData.getRet().equals(RetNumber.ERROR)) {
			...
			statistic = Statistic.GSSO_RECEIVED_USMP_INQUIRYSUBSCRIBER_RESPONSE_ERROR.getStatistic();

			if (ConfigureTool.isWriteLog(ConfigName.DEBUG_LOG_ENABLED)) {
				/** ADD STAT DEBUG INPUT LOG **/
				this.composeDebugLog.addStatisticIn(Statistic.GSSO_RECEIVED_USMP_INQUIRYSUBSCRIBER_RESPONSE_ERROR.getStatistic());
			}
			...
			statistic = Statistic.GSSO_RECEIVED_USMP_INQUIRYSUBSCRIBER_RESPONSE_REJECT.getStatistic();

			if (ConfigureTool.isWriteLog(ConfigName.DEBUG_LOG_ENABLED)) {
				/** ADD STAT DEBUG INPUT LOG **/
				this.composeDebugLog.addStatisticIn(Statistic.GSSO_RECEIVED_USMP_INQUIRYSUBSCRIBER_RESPONSE_REJECT.getStatistic());
			}
			...
			statistic = Statistic.GSSO_RECEIVED_USMP_INQUIRYSUBSCRIBER_RESPONSE_ABORT.getStatistic();

			if (ConfigureTool.isWriteLog(ConfigName.DEBUG_LOG_ENABLED)) {
				/** ADD STAT DEBUG INPUT LOG **/
				this.composeDebugLog.addStatisticIn(Statistic.GSSO_RECEIVED_USMP_INQUIRYSUBSCRIBER_RESPONSE_ABORT.getStatistic());
			}
		...
		ResponseMessage.returnMessage_Error(origInvokeProcess, rawDatasOutgoing, abstractAF, ec02Instance, listCode, listDescription,
				listOrigInvoke, composeDebugLog, true);
	...
	private void inqSubInitInstanceAndLog(EquinoxRawData equinoxRawData, AbstractAF abstractAF, EC02Instance ec02Instance) {
		...
		appInstance.getMapOrigInvokeEventDetailInput().put(origInvokeProcess, EventLog.INQUIRY_SUBSCRIBER.getEventLog());
